...
**Marketing names** (2026-08-29, everywhere a household actually sees the email — subject line, colored mark chip, plain-text header): Urgent alert → **Last Call**, Weekly digest → **Shortlist**, Fortnight look-ahead → **Second Chance**. The names above (Weekly digest/Fortnight look-ahead/Urgent alert) stay as the internal/architectural terms in this doc and in code comments — only the user-facing strings (`cli.py` subject lines, `mark_suffix` in `alert.py`/`digest.py`/`lookahead.py`, plain-text headers) changed. Brand/type separator is always an em dash, applied consistently in every user-facing string: `Curtain Up — <Type>` (chip), `Curtain Up — <Type> — <description>` (subject line, plain-text header) — previously the subject lines omitted the first dash while the chip had it, an inconsistency the user caught by eye in their inbox.
