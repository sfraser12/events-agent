...
- **Per-event "+ calendar" quick-add link** (added 2026-09-01, `cta_cell` in `email_design.py`) - every dated event across all three emails (Shortlist, Understudy, and Last Call - see the 2026-09-03 fix below) carries a one-click Google Calendar link next to Book/"link not working?", using the plain `calendar.google.com/calendar/render?action=TEMPLATE&...` URL scheme rather than the Calendar API - no OAuth, same no-auth-flow-to-maintain spirit as the `.ics` export, just faster for adding a single event without importing a whole file. No `event_date_end` in the digest/lookahead/alert dataclasses, so it defaults to a 3-hour duration; idea borrowed from a SerpApi build-your-own-event-agent tutorial researched 2026-09-01 while looking into Eventbrite coverage options (see the "Eventbrite coverage" note under Phase 7 below) - the one part of that pattern actually worth taking, everything else there was less mature than what's already built here (no persistent memory/dedupe, fetching and judging blended into one interactive LLM session, no hard constraints in code). **Fixed 2026-09-03:** Last Call was missing the link entirely - `alert.py`'s `AlertItem` never carried `event_date` through to `cta_cell`, on the (checked-in-good-faith-but-wrong) assumption that an on-sale/low-availability change has no event date to build a link from. It does: `find_alertable_changes()`'s own query already selects `e.event_date` (needed for the blackout-dates constraint check), it just wasn't being threaded into the dataclass. Caught by the user asking directly whether Last Call had the link, not by testing - worth remembering that "not applicable to X" claims made when a feature ships are worth re-checking rather than trusting at face value.
- **Welcome Shortlist** (added 2026-09-03, `events-agent welcome --household <id>`) - the default onboarding step for a newly configured household, prompted by onboarding household #2 (Ross, Edinburgh). Builds a normal Shortlist from whatever's already been harvested (not waiting for the household's first real Sunday cycle) with a welcome banner (`_welcome_banner_html` in `digest.py`, reusing `build_digest_html`'s new optional `welcome_blurb` param) explaining what Curtain Up sends and why. Sent to the household's own `email_to`, and always also copied to `admin_email` so whoever set the household up can see what it looked like. Logged in `email_log` as a normal `shortlist` row (not a separate type) - it's the same email shape, and it needs to satisfy the Understudy gate below the same way a real one would. **First real send (Ross, 2026-09-03) surfaced a volume problem, not yet fixed:** 725 events in one email - his own permissive `digest_threshold` (50) plus zero prior verdicts (brand new, nothing ever pruned) plus the 270-day "announced for later" horizon (538 of the 725) meant his entire matched backlog went out at once, not a curated preview. Not unique to `welcome` specifically - Shortlist always resends the full standing list, not a delta, so his first real Sunday send would have hit the identical problem with or without this command. Worth a real fix before the next household onboards: cap `welcome` (and arguably a household's very first real Shortlist) to nearer horizons only, or top-N by score, rather than the full multi-hundred-day backlog.
- **Understudy requires a prior Shortlist, 48h apart** (added 2026-09-03, `cmd_fortnight`) - a freshly onboarded household with a large newly-scored backlog could otherwise get Understudy before ever receiving a Shortlist, or immediately after one. `cmd_fortnight` now checks `db.last_email_sent_at()` for the household's most recent `shortlist` row and skips (printing why) if none exists yet or it was under `MIN_HOURS_BETWEEN_SHORTLIST_AND_UNDERSTUDY` (48) hours ago. Applied uniformly, not just to new households - harmless for an established one since real Shortlist/Understudy sends are already days apart on the normal weekly/fortnightly schedule.
- **Admin stats email** (added 2026-08-31, via `events-agent status`) — not household-facing, never sent to `household.email_to`; goes to `Secrets.admin_email` (`ADMIN_EMAIL` in `.env`, falls back to `SMTP_USER`) so it can never accidentally multiply the moment a second household exists. Built to actually answer "how is this scaling, what does it cost, do we need to rearchitect" from real data rather than scrollback: `source_run` counts per adapter over the last 7 days (Ticketmaster/Skiddle/Google Alerts call volume — the practical concern there is the 5,000/day Ticketmaster ceiling, not $ cost, since both APIs are free), `llm_usage` totals per model with an estimated USD cost (Sonnet 5 pricing confirmed via the `claude-api` skill, not guessed — a model missing from the pricing table reports tokens with no cost estimate rather than a wrong number), a same-window breakdown by household/context, and an all-time catalog snapshot (event/venue counts, DB file size). Also carries a **Totals** section (added 2026-08-31, extended 2026-09-01) summing LLM spend, source API calls, and **emails sent** over three windows — This week (7 days), This month (30 days, rolling, not calendar-month-to-date), and All-time — so scale/cost trends are visible without waiting for history to build up in the detailed 7-day breakdown below it. Email counts come from a new `email_log` table (`db.record_email_sent`, written at every real send site — `cmd_digest`/`cmd_fortnight`/`cmd_alert`/`cmd_status` itself) that didn't exist before 2026-09-01 — until then "how many emails go out" had no real answer at all. The catalog snapshot got the same multi-user-readiness treatment: **households configured now reads "N of M defined slots"** (`M` = `len(HOUSEHOLD_IDS)`) rather than just a bare count, so headroom is visible before a second household ever exists, and the event count now carries a **status breakdown** (`on_sale`/`past`/`cancelled`/`announced`/`low_availability`) instead of one opaque total. **Google Alerts content yield section (added 2026-09-01)** — per-feed, all-time, sorted weakest-first: total real events ever produced (from `event_source`, not raw `source_run` fetch-attempt counts, which are cheap and don't mean content was actually found) and days since the last one. Built after a manual audit found the 13 live wellness/spa feeds, running cleanly with zero errors for 4-6 days, had produced a grand total of one row — and that row wasn't even a real event, just a "best hotels" travel listicle that happened to name a venue. Confirmed live against the raw feed XML directly (not just this project's DB) that they're genuinely empty, not a parsing bug on our end. This section makes that an ongoing daily check instead of a one-off manual query — see the "Google Alerts value" note under Phase 7 below for the fuller writeup. Piggybacks on an existing job rather than getting its own plist/wake — cadence is deliberately adjustable: **daily for now** (`run_daily.sh`, right after `events-agent run`, added 2026-08-31 while getting a feel for real spend before dialing back); was briefly on the Sunday weekly job (`run_weekly.sh`, right after `digest`) and can move back there (removing it from `run_daily.sh` at the same time, to avoid a double-send) once daily granularity stops being needed. **Coverage gaps queued section (added 2026-09-03)** — a count (plus per-category breakdown) of rows in `google_alerts_todo.csv` not yet actually created at google.com/alerts (i.e. not `LIVE`/`MOVED`/`DECLINED`). That file is where a taste-profile.md category with no natural source coverage gets logged once found (the same Phase 6/7 research pattern used throughout this project) — it only has value if someone looks at it again, so this surfaces it as an ongoing reminder in the email that's already about "what needs attention" instead of letting it go stale. `_compute_pending_alerts` in `status.py`; skipped entirely if the file is missing or every row is already resolved, same "missing data never excludes" treatment as `annual_anchors.py`.
...
-- One row per email actually sent (added 2026-09-01, feeds the admin stats
-- email's Totals/breakdown sections — see Stage 4). household_label is
-- NULL for the admin stats email itself, never household-scoped.
CREATE TABLE email_log (
    id              INTEGER PRIMARY KEY,
    household_label TEXT,
    email_type      TEXT NOT NULL,   -- shortlist | understudy | last_call | admin_stats
...
- **Design needed: self-service verdict-setting / calendar export for a
  ...
  welcome email originally described "mark anything 'no' and it won't come
  back, mark 'interested'/'booked' and it'll show up in your calendar
  export" as if these were self-service actions — that text is accurate
  for Scott (who has CLI access and already replies-or-runs
  `events-agent verdict`/`events-agent calendar` himself) but not for Ross
  or any future household member without shell access to this machine.
  Removed from the welcome blurb (`WELCOME_BLURB` in `cli.py`) since it
  described a capability the recipient can't actually use unassisted. The
  underlying concept (mark an event, have it stop reappearing or start
  showing up in a calendar export) is sound and worth keeping — it just
  needs an actual delivery mechanism for someone who isn't Scott: options
  worth weighing include reply-to-email parsing, a per-event tokenized
  link in the email itself (no login, similar spirit to the existing
  no-OAuth `+ Calendar` quick-add links), or a small shared script Scott
  hands to each household member. Not designed or built yet.

- **Scoring cost: evaluate a local LLM — flagged 2026-09-03, not yet
  researched.** Recurring concern since the very first cost incident
  (2026-08-25, "we have spent $0.34 on API calls this morning... would a
  local llm help?"), resurfaced harder onboarding Ross: his first-ever
  full-backlog scoring pass (~3,500 candidates) cost **~$3.11 in real
  Anthropic API spend** (org credits $19.90 → $16.79 over the run).
  Ongoing per-household weekly cost is much smaller (a handful of
  new/changed events, ~$0.64/day observed for Scott's account across all
  households combined per the admin stats email) — the expensive case is
  specifically a large first-time backlog, which happens once per
  household but is real money each time (a third household would repeat
  this). Worth investigating whether a local model (Ollama/llama.cpp
  running something like Llama 3/Qwen locally) could handle this
  structured "score 0-100 + short reason against a taste profile, strict
  JSON out" task at acceptable quality for a fraction of the cost,
  especially for the big one-off backlog case where quality-per-event
  matters less than staying within budget. Not researched yet — needs a
  real quality comparison (does a local model's scoring/reasoning hold up
  against Sonnet 5 on this specific taste-matching task) before deciding,
...
- **Source gaps surfaced while writing `taste-profile.md`.** Currently the
  ...
  (tech/space/environment talks — **re-researched 2026-09-01**: confirmed
  Eventbrite's public event-search API was shut down for good in Dec 2019/
  Feb 2020, and the only stated path back to cross-organiser public event
  data is applying to their Distribution Partner Program — no self-serve
  application found, and its one public example (SeatGeek) signals it's
  built for platforms operating at that scale, not a personal project. The
  Google Alerts `site:eventbrite.co.uk/e/` rows already queued in
  `google_alerts_todo.csv` remain the real path here, same as everything
  else in this table — **parked, not pursued further, 2026-09-01: Google's
  own Events search results (surfaced via a paid third-party API, e.g.
  SerpApi's Google Events endpoint — ~£/$0.01-0.025 per search past a
  100-search/month free tier, returns structured title/date/venue/price/
  link fields) would plausibly also catch Eventbrite listings Google has
  indexed, without touching eventbrite.com directly. Worth a real look if
  the Google Alerts rows turn out to have thin coverage once live — no
  adapter spike done yet, deliberately shelved rather than built.**
  **Second parked idea, 2026-09-01: register a real Eventbrite account
  against a dedicated inbox and monitor its "recommended events near you"
  emails** (confirmed real — Eventbrite does send location/behaviour-based
  recommendation emails to registered users; manage via Account Settings →
  Communication Preferences). Technically buildable without violating the
  no-OAuth-flow-to-maintain principle — Gmail IMAP with an app-specific
  password is the same credential shape already used for `SMTP_PASSWORD`,
  not the Calendar-style OAuth this project has otherwise avoided. Parked
  because the content itself is weaker than Google Alerts on the actual
  goal: it's Eventbrite's own promotion-driven algorithm (biased toward
  organisers paying for visibility, the opposite of taste-profile.md's
  "even small/lesser-known organisers" ask), the raw marketing HTML has no
  stability guarantee to parse against (unlike Google Alerts' Atom feed,
  itself only confirmed stable after real verification work), and a mostly-
  dormant account likely produces thin content without genuine browsing
  activity to feed the algorithm. Worth revisiting as a *complement* to
  Google Alerts, not instead of it, if Google Alerts coverage turns out
  thin once live (see the dedicated Google Alerts value-check task below).);
  Facebook Events (one-off local
  ...
  workflow.
- **Google Alerts value-check — done as a manual audit 2026-09-01, now an
  ongoing daily check.** Prompted by "how valuable is Google Alerts to us
  in reality" — the 13 live wellness/spa feeds (see above), running cleanly
  with zero errors for 4-6 days, had produced a grand total of **one row**,
  and even that wasn't a real event (a "10 best hotels for fall foliage"
  travel listicle that happened to name Cameron House). Verified this isn't
  a bug on our end: fetched several of the raw feed XML files directly from
  Google (not through this project's code) and confirmed they're valid,
  well-formed, genuinely empty Atom feeds — Google simply hasn't matched
  fresh content to these specific search terms yet. Doesn't mean Google
  Alerts as a *strategy* is weak — the other ~30 queued `google_alerts_
  todo.csv` rows (Eventbrite topics, fire/Viking festivals, equestrian
  venues, "an evening with" searches) haven't been tested at all and could
  behave completely differently — but it does mean the wellness/spa
  category specifically has produced ~nothing usable so far. Rather than a
  one-off check to remember to re-run, this is now the admin stats email's
  **Google Alerts content yield** section (see above) — a live, ongoing
  answer instead of a stale snapshot. No firm cutoff decided yet for when
  to call a feed dead and delete it from config.
- **Second household.** Brother (Edinburgh) becomes household #2.
  ...
  **Harvest is per-household now (fixed 2026-08-31, was a known gap):**
  ...
  change with only one household configured. **Done 2026-09-03:**
  `households/brother/config.yaml` + `taste-profile.md` created (Ross's own
  write-up, single-user household, very different taste shape from Scott's
  — electronic/jazz/classical, whisky, food & drink, business/finance
  talks, exhibitions; hard no on arena-scale gigs and spectator sport).
  Radius set to 55mi normal / 100mi far tier centred on Hopetoun Street,
  Edinburgh — deliberately not a flat 90mi copy of Scott's config, since
  his own document explicitly treats Dundee (~60mi) as "special trip,
  needs to be exceptional" rather than everyday territory, which a flat
  90mi radius would have contradicted. `price_ceiling` left as a high
  safety net (500) rather than his stated "£100 soft ceiling" verbatim —
  `constraints.py`'s price_ceiling is a hard pre-LLM cutoff by design (see
  Architecture above), so a literal 100 would have silently excluded
  exactly the above-ceiling "strong match, flag the price" events his
  taste-profile.md explicitly says should still surface; the soft-ceiling
  behaviour instead comes from his taste-profile.md's own scoring-band
  text, read by the LLM at scoring time. `digest_threshold` set to 50, not
  ...
  DB checks that whisky tastings/festivals (0-2 real matches — only an
  unrelated "Scottish National Whisky Festival" was covered; the real
  Whisky Fringe and Edinburgh's Whisky Festival at Murrayfield were both
  missing entirely), food & drink (restaurant openings/pop-ups/festivals,
  0 matches — same undated/priceless shape as the existing wellness/spa
  gap), business/finance talks (effectively 0 real matches), and
  exhibitions (2 matches, thin — most gallery/museum exhibitions are
  free/unticketed so wouldn't appear via Ticketmaster/Skiddle regardless,
  not yet researched for a proper feed the way Phase 6 did for venues) are
  all real gaps for his taste profile specifically, distinct from Scott's
  music-heavy gap list. 9 new Google Alert candidate rows added; not yet
  created (waiting on Ross's own google.com/alerts account access, same as
  every other pending row in that file).
