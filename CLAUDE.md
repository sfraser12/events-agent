...
- **Per-event "+ calendar" quick-add link** (added 2026-09-01, `cta_cell` in `email_design.py`) — every dated event in the Shortlist and Understudy emails (not the urgent alert, which has no event date to build one from) carries a one-click Google Calendar link next to Book/"link not working?", using the plain `calendar.google.com/calendar/render?action=TEMPLATE&...` URL scheme rather than the Calendar API — no OAuth, same no-auth-flow-to-maintain spirit as the `.ics` export, just faster for adding a single event without importing a whole file. No `event_date_end` in the digest/lookahead dataclasses, so it defaults to a 3-hour duration; idea borrowed from a SerpApi build-your-own-event-agent tutorial researched 2026-09-01 while looking into Eventbrite coverage options (see the "Eventbrite coverage" note under Phase 7 below) — the one part of that pattern actually worth taking, everything else there was less mature than what's already built here (no persistent memory/dedupe, fetching and judging blended into one interactive LLM session, no hard constraints in code).
