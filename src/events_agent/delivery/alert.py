...
def build_alert_html(household: dict[str, Any], items: list[AlertItem], now: datetime) -> str:
    ...
    return shell(eyebrow="Act now", eyebrow_color=URGENT, eyebrow_bg=URGENT_BG, subtitle=subtitle, body_rows=rows, footer=footer)
...
def build_alert_plain(household: dict[str, Any], items: list[AlertItem], now: datetime) -> str:
    lines = [f"ROUNDUP ALERT — for {household['label']}", ""]
    if not items:
